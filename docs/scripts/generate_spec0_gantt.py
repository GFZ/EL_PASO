...
# Only releases from the last few years can ever be "the current SPEC-0
# floor" (that only depends on the last ~36 months of releases). Restricting
# to this window also sidesteps a real PyPI data quirk: very old package
# versions were sometimes re-uploaded to PyPI's infrastructure long after
# their actual release, so their recorded upload time is not chronological
# relative to later versions. That noise lives well outside this window.
RELEASE_LOOKBACK_YEARS = 6

# The packages SPEC-0 publishes a drop schedule for, restricted to the ones
# el_paso actually depends on at runtime.
